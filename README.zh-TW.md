...
`everyone` 也可作為 `each` 的 alias，但建議使用 `each`。

server 會依照 `MYSQL_BATCH_MAX_SIZE` 將 `paramsList` 切成內部分批。例如預設大小 `100` 時，`250` 組參數會切成 `100`、`100`、`50`。

詳細的逐筆執行結果會寫到 `logs/` 底下具時間戳記的 `.log` 檔。tool response 只會回傳摘要數字與 log file path。server 啟動時會自動清理超過七天的 log files。
...
- batch execution 與 CSV import logs 會包含參數值與逐筆結果。請將 `logs/` 下的檔案視為敏感資料。
